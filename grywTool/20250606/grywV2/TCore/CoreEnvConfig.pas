...
type
  ...
  private
    ...
    //FResults: TDictionary<TItemType, Boolean>;        //检测类型-是否完成
...
type
  ...
  private
    ...
    //function DeleteReg(const RegPath: string): Boolean;
    //procedure ExtractPermission(const FilePath: string; const Parameters: string);
  public
  ...
  TOtherChecker = class(TEnvChecker)
  private
    SQLCompleted: Boolean;
    TubesCompleted: Boolean;
    ProgramCompleted: Boolean;
    //function DeleteTubes: Boolean;
    //function DeleteProgram: Boolean;
    //function DeleteFile(const FilePath: string): Boolean;
  public
    function Check(): Boolean; override;
    function Fix(): Boolean; override;
...
begin
  ...
  begin
    try
      ...
      until Integer(FindNext(SearchRec)) <> 0;
    finally
...
begin
  ...
  //FResults := TDictionary<TItemType, Boolean>.Create;
  InitCheckers;
end;
...
begin
  ...
  FCheckers.Add(EnvOther, TOtherChecker.Create);

  //for I := Low(TItemType) to High(TItemType) do
    //FResults.Add(I, False);
end;
...
{ TOtherChecker }
function TOtherChecker.Check: Boolean;
var
  LocalPath: string;
  CloudPath: string;
  Reg: TRegistry;
...
function TOtherChecker.Fix: Boolean;
begin
