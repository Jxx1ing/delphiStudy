...
uses
  ...
  Math, StrUtils, HttpGet, Vcl.ComCtrls, Vcl.Imaging.pngimage;
...
type
  TFrmNet = class(TFrame)
    ...
    procedure scrlbxNetMouseWheel(Sender: TObject; Shift: TShiftState; WheelDelta: Integer; MousePos: TPoint; var Handled: Boolean);
  private
    FNetManger: TNetManger;
    FBasicConnetDic: TDictionary<Integer, TDisplayItem>;
    FLatencyAndLossDic: TDictionary<Integer, string>;
    FDNSAndProxyDic: TDictionary<Integer, TDisplayItem>;
...
const
  ...
  DNSSETTINGFALSE = '错误，请尝试以管理员权限修复';
  SAASLATENCYANDLOSS = 'SaaSServer 延迟 / 丢包率';
  CLOUDLATENCYANDLOSS = 'CloudServer 延迟 / 丢包率';
  DNSFIXSUCCESSED = '修复成功: 114.114.114.114/223.5.5.5';
  DNSFIXFAILED = '修复失败: 请参考手册4.2';
...
begin
  ...
  FBasicConnetDic := TDictionary<Integer, TDisplayItem>.Create;
  for I := 0 to lstBasicConnet.ItemCount - 1 do
    FBasicConnetDic.Add(I, TDisplayItem.Create('', False));

  // 测试网络配置信息
  lstConfig.ItemCount := 2;
  FDNSAndProxyDic := TDictionary<Integer, TDisplayItem>.Create;
  for I := 0 to lstConfig.ItemCount - 1 do
    FDNSAndProxyDic.Add(I, TDisplayItem.Create('', False));
...
begin
  ...
  if FBasicConnetDic.TryGetValue(AIndex, Item) then
  begin
    lblBasicConnectValue.Caption := Item.Text;
    lblBasicConnectValue.Font.Color := IfThen(Item.IsPositive, clGreen, clRed);
  end;
end;
...
begin
  ...
  if FDNSAndProxyDic.TryGetValue(AIndex, Item) then
  begin
    lblConfigValue.Caption := Item.Text;
    lblConfigValue.Font.Color := IfThen(Item.IsPositive, clGreen, clRed);
  end;
end;

procedure TFrmNet.lstLatencyAndLossBeforeDrawItem(AIndex: Integer; ACanvas: TCanvas; ARect: TRect; AState: TOwnerDrawState);
var
  LatencyStr, LossStr: string;
  Latency: Integer;
  Loss: Double;
begin
  case AIndex of
  ...
  // 提取延迟和丢包率数值
  LatencyStr := Copy(FLatencyAndLossDic[AIndex], Pos('延迟: ', FLatencyAndLossDic[AIndex]) + 4, Pos(' ms', FLatencyAndLossDic[AIndex]) - (Pos('延迟: ', FLatencyAndLossDic[AIndex]) + 4));
  LossStr := Copy(FLatencyAndLossDic[AIndex], Pos('丢包: ', FLatencyAndLossDic[AIndex]) + 4, Pos('%', FLatencyAndLossDic[AIndex]) - (Pos('丢包: ', FLatencyAndLossDic[AIndex]) + 4));

  Latency := StrToIntDef(LatencyStr, 999);
  Loss := StrToFloatDef(LossStr, 100.0);

  // 综合判定颜色
  if (Latency >= 150) or (Loss > 5) then
    lblLatencyAndLossValue.Font.Color := clRed
  else if (Latency >= 50) or (Loss > 1) then
    lblLatencyAndLossValue.Font.Color := clYellow
  else
    lblLatencyAndLossValue.Font.Color := clGreen;
end;

procedure TFrmNet.scrlbxNetMouseWheel(Sender: TObject; Shift: TShiftState; WheelDelta: Integer; MousePos: TPoint; var Handled: Boolean);
begin
  scrlbxNet.VertScrollBar.Position := scrlbxNet.VertScrollBar.Position - WheelDelta div 2;
  Handled := True;
end;
...
begin
  ...
  begin
    FBasicConnetDic[Ord(SaaSLink)] := TDisplayItem.Create(IfThen(BasicConnectChecker.SaaSConnected, CONNECTSUCCESSED, CONNECTFAILED), BasicConnectChecker.SaaSConnected);

    FBasicConnetDic[Ord(CloudLink)] := TDisplayItem.Create(IfThen(BasicConnectChecker.CloudConnected, CONNECTSUCCESSED, CONNECTFAILED), BasicConnectChecker.CloudConnected);

    FBasicConnetDic[Ord(InternetLink)] := TDisplayItem.Create(IfThen(BasicConnectChecker.InternetConnected, CONNECTSUCCESSED, CONNECTFAILED), BasicConnectChecker.InternetConnected);

    lstBasicConnet.Repaint;
  end;
end;
...
begin
  ...
  begin
    FDNSAndProxyDic[Ord(DNSconfig)] := TDisplayItem.Create(IfThen(ConfigChecker.DNSIsTrue, DNSSETTINGTRUE, DNSSETTINGFALSE), ConfigChecker.DNSIsTrue);

    FDNSAndProxyDic[Ord(Proxyconfig)] := TDisplayItem.Create(IfThen(ConfigChecker.ProxyOpened, PROXYOPENING, PROXYCLOSEING), not ConfigChecker.ProxyOpened); // 代理开启是负面状态
...
begin
  FDNSAndProxyDic[Ord(DNSconfig)] := TDisplayItem.Create(IfThen(FNetManger.Fix, DNSFIXSUCCESSED, DNSFIXFAILED), FNetManger.Fix);
  lstConfig.Repaint;
end;
